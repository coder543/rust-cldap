...
impl RustLDAP {
    ...
    /// subsequent calls to `set_option` and `simple_bind_authentication` are possible. Before calling a search
    /// related function, one must bind to the server by calling `simple_bind_authentication`. See module usage
